...
int main(){
    ...
    sort(tmp,tmp+a);
    for(int i=0;i<b;i++) scanf("%d%d%d",A+i,B+i,C+i);
    ...
    for (int i=0;i<b;i++) if(A[i]+B[i]>=C[i]) ev.push_back(make_pair(A[i],i));
    ...
    for(int i=0;i<b;i++) if(A[i]+B[i]<C[i]) ev.push_back(make_pair(C[i],i));
