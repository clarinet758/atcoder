...
int main(){
    ...
    rep(i,h) {
        cin >> p.at(i);
        if(i &&  p.at(i).at(0)!='#') dp.at(i).at(0)=dp.at(i-1).at(0);
    }
    rep(i,w) if(i &&  p.at(0).at(i)!='#') dp.at(0).at(i)=dp.at(0).at(i-1);
    for(int i=1;i<h;i++){
        for(int j=1;j<w;j++){
            if(p.at(i).at(j)!='#') dp.at(i).at(j)=dp.at(i-1).at(j)+dp.at(i).at(j-1);
            dp.at(i).at(j)%=mod;
        }
    }
