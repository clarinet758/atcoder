...
int main(){
    ...
    rep(i,s.size()){
        if(i==0 || a.back().first!=s.at(i)-'a') a.push_back({s.at(i)-'a',0});
        a.back().second++;
    }
    rep(i,t.size()){
        if(i==0 || b.back().first!=t.at(i)-'a') b.push_back({t.at(i)-'a',0});
        b.back().second++;
    }
    if(a.size()!=b.size()) ans=0;
    else{
        rep(i,a.size()){
            if(a.at(i).first!=b.at(i).first) ans=0;
            else{
                if(a.at(i).second==b.at(i).second && b.at(i).second==1){
                    1; 
                }else if(a.at(i).second>1 && b.at(i).second>1){
                    1; 
                }else{
                    ans=0;
                }
