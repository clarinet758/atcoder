...
int main(){
    ...
    rep(i,n) { cin >> a.at(i).first; a.at(i).second=i; c.at(i).first=a.at(i).first; }
    rep(i,n) { cin >> b.at(i).first; b.at(i).second=i; c.at(i).second=i; c.at(i).first+=b.at(i).first; a.at(i).first*=-1; b.at(i).first*=-1; c.at(i).first*=-1;}
    sort(a.begin(),a.end());
    sort(b.begin(),b.end());
    sort(c.begin(),c.end());
    rep(i,n){
        if(ans.size()==x) break;
        if(p.at(a.at(i).second)==0) { p.at(a.at(i).second)=1; ans.push_back(a.at(i).second); }
    }

    rep(i,n){
        if(ans.size()==x+y) break;
        if(p.at(b.at(i).second)==0) { p.at(b.at(i).second)=1; ans.push_back(b.at(i).second); }
    }

    rep(i,n){
        if(ans.size()==x+y+z) break;
        if(p.at(c.at(i).second)==0) { p.at(c.at(i).second)=1; ans.push_back(c.at(i).second); }
    }
    sort(ans.begin(),ans.end());
    rep(i,ans.size()) cout << ans.at(i)+1 << endl;
    return 0;
}