...
int main(){
    int mod=1e9+7;
    int n,k,h,w,x=0,y=0,z,cnt=0,ans=1;
    cin >> h >> w;
    string s,t;
    vector<string> u(h);
    vector<vector<bool>> p(h,vector<bool>(w,0));
    p[0][0]=1;
    rep(i,h) cin >> u.at(i);
    for(;;){
        if(u[y][x]=='U' && y>0) y--;
        else if(u[y][x]=='D' && y<h-1) y++;
        else if(u[y][x]=='R' && x<w-1) x++;
        else if(u[y][x]=='L' && x>0) x--;
        else break;
        if(p[y][x]) {cout << -1 << endl; return 0;}
        p[y][x]=1;
    }
    cout << y+1 << " " << x+1 << endl;
    return 0;
