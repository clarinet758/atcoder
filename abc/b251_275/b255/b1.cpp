...
int main(){
    ...
    rep(i,n) cin >> w.at(i).first >> w.at(i).second;

    rep(i,n){
        x=w.at(i).first; y=w.at(i).second;
        f=1;
        rep(j,k){
            if(i==a.at(j)) {
                f=0;
            }else{
                xx=w.at(a.at(j)).first; yy=w.at(a.at(j)).second;
                xx=abs(x-xx); yy=abs(y-yy);
                cnt=pow(xx*xx+yy*yy,0.5);
            }
        }
