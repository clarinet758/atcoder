...
int main(){
    ...
    rep(i,n-2) for(int j=i+1;j<n-1;j++) for(int k=j+1;k<n;k++){
        vector<pair<int,int>> p(3);
        p.at(0).first=a.at(i).first;
        p.at(0).second=a.at(i).second;
        p.at(1).first=a.at(j).first;
        p.at(1).second=a.at(j).second;
        p.at(2).first=a.at(k).first;
        p.at(2).second=a.at(k).second;
        sort(p.begin(),p.end());
        if(p.at(0).first==p.at(1).first && p.at(1).first==p.at(2).first) ans=1;
        else if(p.at(0).second==p.at(1).second && p.at(1).second==p.at(2).second) ans=1;
        else{
            int a,b,c,d;
            a=p.at(1).first-p.at(0).first;
            b=p.at(1).second-p.at(0).second;
            c=p.at(2).first-p.at(1).first;
            d=p.at(2).second-p.at(1).second;
            if(b*c==d*a) ans=1;
        }
    }
