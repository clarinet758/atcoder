...
int main(){
    int n,x,y;
    ll ans=0ll,m=998244353;
    cin >> n;
    vector<vector<ll>> a(2,(vector<ll>(9,1ll)));
    rep(i,n) {
        rep(j,9){
            if(i%2) x=0,y=1;
            else x=1,y=0;
            if(j==0){
                a.at(x).at(0)=a.at(y).at(0)+a.at(y).at(1);
            }else if(j==8){
                a.at(x).at(8)=a.at(y).at(8)+a.at(y).at(7);
            }else{
                a.at(x).at(j)=a.at(y).at(j)+a.at(y).at(j+1)+a.at(y).at(j-1);
            }
            a.at(x).at(j)%=m;
