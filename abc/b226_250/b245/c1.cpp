...
int main(){
    int mod=1e9+7;
    int n,k,x=1,y=2,xx=0,yy=0,z,cnt=0,ans=0;
    cin >> n >> k;
    vector<int> a(n);
    vector<int> b(n);
    rep(i,n) cin >> a.at(i);
    ...
    rep(i,n-1){
        if(x==1){
            if(abs(a.at(i)-a.at(i+1))<=k) xx+=1;
            if(abs(a.at(i)-b.at(i+1))<=k) xx+=2;
            if(xx==0) xx=-1;
        }
        if(x==2){
            if(abs(b.at(i)-a.at(i+1))<=k) xx+=1;
            if(abs(b.at(i)-b.at(i+1))<=k) xx+=2;
            if(xx==0) xx=-1;
        }
        if(x==3){
            if((abs(a.at(i)-a.at(i+1))<=k)||(abs(b.at(i)-a.at(i+1))<=k)) xx+=1;
            if((abs(a.at(i)-b.at(i+1))<=k)||(abs(b.at(i)-b.at(i+1))<=k)) xx+=2;
            if(xx==0) xx=-1;
        }
        if(x>0)x=xx;
        xx=0;
        ...
        if(y==1){
            if(abs(a.at(i)-a.at(i+1))<=k) yy+=1;
            if(abs(a.at(i)-b.at(i+1))<=k) yy+=2;
            if(yy==0) yy=-1;
        }
        if(y==2){
            if(abs(b.at(i)-a.at(i+1))<=k) yy+=1;
            if(abs(b.at(i)-b.at(i+1))<=k) yy+=2;
            if(yy==0) yy=-1;
        }
        if(y==3){
            if((abs(a.at(i)-a.at(i+1))<=k)||(abs(b.at(i)-a.at(i+1))<=k)) yy+=1;
            if((abs(a.at(i)-b.at(i+1))<=k)||(abs(b.at(i)-b.at(i+1))<=k)) yy+=2;
            if(yy==0) yy=-1;
        }
        if(y>0)y=yy;
        yy=0;
