...
int main(){
    int mod=1e9+7;
    int h,w,x,y,xx,yy,z,cnt=0,ans=0;
    cin >> h >> w;
    string s,t;
    ...
    rep(i,h) {
        rep(j,w) {
            ...
            if(cnt==0 && a.at(y).at(x)%2) {xx=x; yy=y; cnt=1;}
            else if(cnt==1 && a.at(y).at(x)%2) {ans++; p.push_back({yy+1,xx+1,y+1,x+1}); cnt=0;}
            else if(cnt==1 && a.at(y).at(x)%2==0) {ans++; p.push_back({yy+1,xx+1,y+1,x+1}); xx=x; yy=y;}
        }
    }
    cout << ans << endl;
    rep(i,ans) {
        cout << p.at(i).at(0) << " " << p.at(i).at(1) << " " << p.at(i).at(2) << " " << p.at(i).at(3) << endl;
    }
    return 0;
}