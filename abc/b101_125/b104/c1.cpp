...
int main(){//後で
    int mod=1e9+7;
    int d,g,n,k,x,y,z,cnt=0,ans=mod;
    cin >> d >> g;
    //cin >> m;
    vector<vector<pair<int,int>>> a(10,vector<pair<int,int>>(102));
    rep(i,d) {
        cin >> x >> y;
        rep(j,x+1) {
            a.at(i).at(j).first=(i+1)*j*100;
            a.at(i).at(j).second=j;
            a.at(i).at(x).first+=y;
        }
    }
    vector<int> w(1e7,mod);
    rep(i,d){
        for(int j=100;j>=0;j--){
    ...
    rep(i,2){
        rep(j,7){
            if(j<5) cout << a.at(i).at(j).first << a.at(i).at(j).second << " ";
            else cout << a.at(i).at(j).first << a.at(i).at(j).second << endl;
        }
    }
    **/
