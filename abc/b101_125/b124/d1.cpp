...
int main(){
    ...
    y=s.at(0)-'0';
    rep(i,n) {
        if(s.at(i)-'0'==y && y==0 ) a.at(x)--;
        else if(s.at(i)-'0'==y && y==1 ) a.at(x)++;
        else if(s.at(i)-'0'!=y && y==0 ) {x++; a.at(x)++; y=1; }
        else if(s.at(i)-'0'!=y && y==1 ) {x++; a.at(x)--; y=0; }
    }
    rep(i,x+1){
        if(a.at(i)<0 && cnt<k) {
            cnt++;
            ans+=abs(a.at(i));
        } else if(a.at(i)<0 && cnt==k) {
            if( q.front()>0) {
                ans-=abs(q.front());
                q.pop();
                ans-=abs(q.front());
