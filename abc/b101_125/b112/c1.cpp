...
int main(){
    ...
    rep(i,n) {
        cin >> a.at(i).at(1) >> a.at(i).at(2) >> a.at(i).at(0);
        a.at(i).at(0)*=-1;
    }
    sort(a.begin(),a.end());
    rep(i,101) rep(j,101) {
        ans=(a.at(0).at(0)*-1)+abs(a.at(0).at(1)-i)+abs(a.at(0).at(2)-j);
        cnt=1;
        rep(p,n){
            if(a.at(p).at(0)==0 && ans>(a.at(p).at(0)*-1)+abs(a.at(p).at(1)-i)+abs(a.at(p).at(2)-j)) {cnt=0; break;}
            else if(a.at(p).at(0)!=0 && ans!=(a.at(p).at(0)*-1)+abs(a.at(p).at(1)-i)+abs(a.at(p).at(2)-j)) {cnt=0; break;}
        }
        if(cnt && ans>0) { cout << i << " " << j << " " << ans << endl; return 0;}
    }
    return 0;
}