...
int main(){
    ...
    rep(i,n*3) {
        cin >> x;
        if (a.at(x-1).first==-1) {a.at(x-1).first=0; a.at(x-1).second=x;}
        else if (a.at(x-1).first==0) a.at(x-1).first=i+1;
    }
    sort(a.begin(),a.end());
    rep(i,n) {
        if (i!=n-1) cout << a.at(i).second << " ";
        else cout << a.at(i).second << endl;
    }
    return 0;
}