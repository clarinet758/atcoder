...
int main(){
    ...
    cin >> n;
    vector<int> a(105);
    vector<vector<int>> w(40,vector<int>());
    vector<vector<int>> p(40,vector<int>());
    vector<int> b;
    rep(i,n) {
        cin >> c;
        a.at(i)=c;
    ...
    if(w.at(x).size()==0){
        ...
        rep(i,w.at(x).size()){
            p.at(a.at(w.at(x).at(i))).push_back(w.at(x).at(i)+1);
        }
        rep(i,38){
            if(p.at(i).size()!=0){
                cout << p.at(i).size() << endl;
                sort(p.at(i).begin(),p.at(i).end());
                rep(j,p.at(i).size()){
                    b.push_back(p.at(i).at(j));
                }
                break;
            }
        }
