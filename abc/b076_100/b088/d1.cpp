...
int main(){
    int mod=1e9+7;
    int h,w,x,y,z,cnt=0,ans=0;
    cin >> h >> w;
    vector<string> s(h);
    rep(i,h) {
        cin >> s.at(i);
        rep(j,w) cnt+=(s.at(i).at(j)=='#');
    }
    
    vector<vector<int>> m(50,vector<int>(50,mod));
    m.at(0).at(0)=0;
    ...
    while(p.size()>0) {
        ...
        x=p.back().second;
        p.pop_back();
        if (y>0 && s.at(y-1).at(x)=='.' && m.at(y-1).at(x)>m.at(y).at(x)+1) {m.at(y-1).at(x)=m.at(y).at(x)+1; p.push_back({y-1,x});}
        if (x>0 && s.at(y).at(x-1)=='.' && m.at(y).at(x-1)>m.at(y).at(x)+1) {m.at(y).at(x-1)=m.at(y).at(x)+1; p.push_back({y,x-1});}
        if (y<h-1 && s.at(y+1).at(x)=='.' && m.at(y+1).at(x)>m.at(y).at(x)+1) {m.at(y+1).at(x)=m.at(y).at(x)+1; p.push_back({y+1,x});}
        if (x<w-1 && s.at(y).at(x+1)=='.' && m.at(y).at(x+1)>m.at(y).at(x)+1) {m.at(y).at(x+1)=m.at(y).at(x)+1; p.push_back({y,x+1});}
    }
    ans=m.at(h-1).at(w-1);
    cout << ((ans==mod)?-1:h*w-ans-cnt-1) << endl;
