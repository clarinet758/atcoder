...
int main(){
    int mod=1e9+7;
    ll n,x1,x2,x3,y,z,cnt=0ll,ans=0ll;
    int k,f=0;
    cin >> n;
    string s,t;
    vector<ll> a(5);
    ...
    rep(i,32){
        k=i,f=0;
        x1=-1ll,x2=-1ll,x3=-1ll;
        rep(j,5){
            if(k&1 && f==0) {x1=a.at(j); f++;}
            else if(k&1 && f==1) {x2=a.at(j); f++;}
            else if(k&1 && f==2) {x3=a.at(j); f++;}
            else if(k&1) f++;
            k=k>>1;
        }
        if(f==3) ans+=(1ll*x1*x2*x3);
