...
int main(){
    ...
    rep(i,n) {
        cin >> a.at(i+1);
        if(i==0) p.at(i+1)=abs(a.at(i+1));
        else p.at(i+1)=p.at(i)+abs(a.at(i)-a.at(i+1));
    }
    p.at(n+1)=p.at(n)+abs(a.at(n));
    ans=p.at(n+1);
    rep(i,n){
        cnt=ans;
        cnt-=(abs(a.at(i+1)-a.at(i))+abs(a.at(i+2)-a.at(i+1)));
        cnt+=abs(a.at(i+2)-a.at(i));
        cout << cnt << endl;
    }
    return 0;
}