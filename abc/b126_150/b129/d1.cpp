...
int main(){
    ...
    cin >> h >> w;
    vector<vector<pair<int,int>>> l(h+2,vector<pair<int,int>>(w+2));
    vector<vector<pair<int,int>>> r(h+2,vector<pair<int,int>>(w+2));
    vector<string> s(h);
    rep(i,h) cin >> s.at(i);

    rep(i,h)rep(j,w) {
        if(s.at(i).at(j)=='.') r.at(i+1).at(j+1).first=r.at(i+1).at(j).first+1;
        if(s.at(i).at(w-j-1)=='.') r.at(i+1).at(w-j).second=r.at(i+1).at(w-j+1).second+1;
    }

    rep(i,w)rep(j,h){
        if(s.at(j).at(i)=='.')l.at(j+1).at(i+1).first=l.at(j).at(i+1).first+1;
        if(s.at(h-j-1).at(i)=='.')l.at(h-j).at(i+1).second=l.at(h-j+1).at(i+1).second+1;
    }

    rep(a,h)rep(b,w) {
        int i=a+1,j=b+1;
        y=max(0,l.at(i).at(j).first+l.at(i).at(j).second-1), x=max(0,r.at(i).at(j).first+r.at(i).at(j).second-1);
        z=x+y-(x>0 && y>0);
        ans= max(ans,z);
    }
    cout << ans << endl;
