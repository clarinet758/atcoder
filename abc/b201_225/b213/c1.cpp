...
int main(){
    ...
    rep(i,n) {
        ...
        if(hh[a.at(i).first]==0) hv.push_back(a.at(i).first);
        if(ww[a.at(i).second]==0) wv.push_back(a.at(i).second);
        hh[a.at(i).first]=a.at(i).first;
        ww[a.at(i).second]=a.at(i).second;
    }
    sort(hv.begin(),hv.end());
    sort(wv.begin(),wv.end());
    rep(i,hv.size()) hh[hv.at(i)]=i+1;
    rep(i,wv.size()) ww[wv.at(i)]=i+1;
    rep(i,n) cout << hh[a.at(i).first] << " " << ww[a.at(i).second] << endl;
    return 0;
}