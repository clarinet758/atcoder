...
int main(){
    int mod=1e9+7;
    int n,m,k,x,y,z,cnt=mod,ans=mod;
    cin >> n >> m;
    vector<int> a(n);
    vector<int> b(m);
    rep(i,n) cin >> a.at(i);
    ...
    sort(b.begin(),b.end());
    rep(i,n){
        auto itr=upper_bound(b.begin(),b.end(),a.at(i));
        x=itr-b.begin();
        if(b.at(x)!=mod) ans=min(ans,abs(a.at(i)-b.at(x)));
        if(b.at(x-1)!=0) ans=min(ans,abs(a.at(i)-b.at(x-1)));
        if(b.at(x+1)!=mod) ans=min(ans,abs(a.at(i)-b.at(x+1)));
    }
    print(ans);
    return 0;
}