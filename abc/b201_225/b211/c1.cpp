...
int main(){
    ...
    rep(i,n) {
        if(i>6){
            x+=a.at(6).at(i-1);
            x%=mod;
            if(s.at(i)=='i') a.at(7).at(i)=x;
        }
    }
    rep(i,n) ans=(ans+a.at(7).at(i))%mod;
    print(ans);
    return 0;
}